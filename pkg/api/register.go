...
func (a *AcmednsAPI) webRegisterPost(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	// Create new user. A domain_name in the payload labels the registration for
	// the management UI; clients that do not send one are unaffected.
	nu, err := a.DB.RegisterWithName(aTXT.AllowFrom, aTXT.DomainName)
	if err != nil {
