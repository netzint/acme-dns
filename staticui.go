...
func uiAvailable() bool {
	info, err := os.Stat(Config.API.UIPath)
	if err != nil || !info.IsDir() {
		return false
	}
	_, err = os.Stat(filepath.Join(Config.API.UIPath, "index.html"))
	return err == nil
}
...
func spaFileServer(root string) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if path.Ext(cleaned) != "" {
			log.WithFields(log.Fields{"path": cleaned}).Debug("UI asset not found")
			http.Error(w, "not found", http.StatusNotFound)
			return
		}
