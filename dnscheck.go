...
func checkResolvers() []string {
	if len(Config.DNSCheck.Resolvers) == 0 {
		return defaultCheckResolvers
	}
	resolvers := make([]string, 0, len(Config.DNSCheck.Resolvers))
	for _, r := range Config.DNSCheck.Resolvers {
...
func txtTimeout() time.Duration {
	if Config.DNSCheck.TXTTimeoutSeconds > 0 {
		return time.Duration(Config.DNSCheck.TXTTimeoutSeconds) * time.Second
	}
...
func webDNSCheck(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	if _, err := DB.GetBySubdomain(req.Subdomain); err != nil {
	...
	result := runDNSCheck(domain, req.Subdomain, req.SkipTXT)
...
func runDNSCheck(domain, subdomain string, skipTXT bool) DNSCheckResponse {
	challenge := "_acme-challenge." + domain + "."
	expected := strings.ToLower(subdomain + "." + Config.General.Domain + ".")
	...
	token := generatePassword(43)
	if err := DB.Update(ACMETxtPost{Subdomain: subdomain, Value: token}); err != nil {
		log.WithFields(log.Fields{"error": err.Error()}).Error("Error writing test TXT value")
		resp.Steps = append(resp.Steps, DNSCheckStep{
			ID:     "txt",
			Status: checkStepStatusFail,
	...
	found, waited := pollTXT(resolvers, challenge, token, txtTimeout())
	if !found {
		resp.Steps = append(resp.Steps, DNSCheckStep{
			ID:     "txt",
			Status: checkStepStatusFail,
